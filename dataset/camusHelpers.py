'''
The helper methods in this file are supplied by the CAMUS challenge and are unchanged.
Source: https://www.creatis.insa-lyon.fr/Challenge/camus/databases.html
'''
...
def resize_image_to_isotropic(
    image: np.ndarray, spacing: Tuple[float, float], resample: Resampling = Resampling.NEAREST) -> np.ndarray:
...
def compute_left_ventricle_volumes(
    ...
    a4c_ed: np.ndarray,
    a4c_es: np.ndarray,
    a4c_voxelspacing: Tuple[float, float]) -> Tuple[float, float]:
